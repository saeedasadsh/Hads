        ...
        if (player != null && userData.type == "or") {
            ...
            for (i = 0; i < leagues.length; i++) {
                if (userData.leagueId == leagues[i]) {
                    ...
                    //var counts = leaguesPlayersArr[i].length;

                    //var data = { counts: counts };


                    //var sc = leagueSocksArr[i];
                    //sc.forEach(function (item, index, arr) {

                    //    item.emit("OnLeaguePlayersCount", data);
                    //});


                    //var countsl = [];
                    //for (j = 0; j < leaguesPlayersArr.length; j++) {
                    //    countsl.push(leaguesPlayersArr[j].length);
                    //}

                    //var datal = { counts: countsl };

                    //for (j = 0; j < leagueSocksArr.length; j++) {
                    //    var sc = leagueSocksArr[j];
                    //    sc.forEach(function (item, index, arr) {

                    //        item.emit("AllLeaguePlayersCount", datal);
                    //    });
                    //}
                }
            }
